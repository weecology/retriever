...
class Engine(object):
    ...
    def download_files_from_archive(self, url, filenames, filetype="zip",
        ...
        for filename in filenames:
            if self.find_file(os.path.join(archivebase, filename)):
                ...
                unzipped_file = open(fileloc, 'wb') #test needed w or wb??
    ...
    def set_table_delimiter(self, file_path):
        if sys.version_info >= (3, 0, 0):
            if os.name=='nt':
                dataset_file = io.open(file_path, 'r', newline='', encoding='ISO-8859-1')
            else:
                dataset_file = io.open(file_path, newline='', encoding='latin-1')
        else:
            dataset_file = io.open(file_path, encoding='ISO-8859-1')
    ...
    def to_csv(self):
        ...
        for item in list(self.script.urls.keys()):
            ...
            csv_file_output = (table_name + '.csv')
            if sys.version_info >= (3, 0, 0):
                csv_out = io.open(csv_file_output, 'w', newline='', encoding='ISO-8859-1')
            else:
                csv_out = io.open(csv_file_output, 'wb')
    ...
    def load_data(self, filename):
        ...
        if sys.version_info >= (3, 0, 0):
            if os.name=="nt":
                dataset_file = io.open(filename, 'r', newline='', encoding='ISO-8859-1')
            else:
                dataset_file = io.open(filename,'r', encoding='ISO-8859-1')
        else:
