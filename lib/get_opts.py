...
install_parser = subparsers.add_parser(
    'install', help='download and install dataset')
update_parser = subparsers.add_parser(
    'update', help='download updated versions of scripts')
gui_parser = subparsers.add_parser(
    'gui', help='launch retriever in graphical mode')
new_parser = subparsers.add_parser(
    'new', help='create a new sample retriever script')
...
reset_parser = subparsers.add_parser(
    'reset', help='reset retriever: removes configation settings, scripts, and cached data')
...
citation_parser.add_argument(
    'dataset', help='dataset name', nargs='?', default=None)
new_parser.add_argument('filename', help='new script filename')
reset_parser.add_argument('scope', help='things to reset: all, scripts, data, or connections', choices=[
                          'all', 'scripts', 'data', 'connections'])
install_parser.add_argument(
    '--compile', help='force re-compile of script before downloading', action='store_true')
install_parser.add_argument(
...
for engine in engine_list:
    ...
    for arg in engine.required_opts:
        ...
        if engine.name == "Download Only" or abbreviation == "download":
            ...
            # subdir doesn't take any arguments, if included takes True if
            # excluded takes False
            if arg_name.lower() == "subdir":
                download_parser.add_argument(
                    '--%s' % arg_name, '-%s' % abbreviation, help=help_msg, default=default, action='store_true')
