...
class main(Script):
    def __init__(self, **kwargs):
        ...
        self.title = (
            "Food web including metazoan parasites for a "
            "brackish shallow water ecosystem in Germany and Denmark"
        )
        self.citation = (
            "C. Dieter Zander, Neri Josten, Kim C. Detloff, "
        ...
        self.description = (
            "This data is of a food web for the Flensburg "
    ...
    def download(self, engine=None, debug=False):
        ...
        engine = self.engine
        file_names = [
            ("Flensburg_Data_Links.csv", "links"),
            ("Flensburg_Data_Nodes.csv", "nodes"),
        ]

        engine.download_files_from_archive(
            self.urls["zip"], [i[0] for i in file_names], "zip", False, "ECOL_92_174"
        )
