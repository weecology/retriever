...
class main(Script):
    def __init__(self, **kwargs):
        ...
        self.version = "1.3.2"
        self.ref = (
            "https://figshare.com/collections/Tree_demography_in_an_undisturbed_"
        ...
        self.citation = (
            "Raphael Pelissier, Jean-Pierre Pascal, N. Ayyappan, B. R. Ramesh, "
        ...
        self.description = (
            "A data set on demography of trees monitored over 20 years in "
    ...
    def download(self, engine=None, debug=False):
        ...
        engine.download_files_from_archive(
            self.urls["data"],
            ["UPSP_Demo_data.txt", "UPSP_Species_list2.txt"],
            filetype="zip",
        )
        # Create table sp_list(Species)
        filename = "UPSP_Species_list2.txt"
        engine.auto_create_table(
            Table("sp_list", cleanup=self.cleanup_func_table), filename=filename
        )
        ...
        engine.auto_create_table(
            Table("ind_loc_girth", cleanup=self.cleanup_func_table), filename=filename
        )
        engine.insert_data_from_file(engine.format_filename(filename))
