...
class main(Script):
    def __init__(self, **kwargs):
        ...
        self.citation = (
            "Falster, D.S., Duursma, R.A., Ishihara, M.I., "
        ...
        self.version = "1.4.4"
        self.description = (
            "The data set is a Biomass and allometry database "
    ...
    def download(self, engine=None, debug=False):
        ...
        if parse_version(VERSION).__str__() >= parse_version("2.1.dev").__str__():
            ...
            engine.auto_create_table(
                Table("data", cleanup=self.cleanup_func_table), filename=filename
            )
        ...
        if parse_version(VERSION).__str__() >= parse_version("2.1.dev").__str__():
            ...
            engine.auto_create_table(
                Table("methods", cleanup=self.cleanup_func_table), filename=filename
            )
            engine.insert_data_from_file(engine.format_filename(filename))
        else:
