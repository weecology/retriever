...
class main(Script):
    def __init__(self, **kwargs):
        ...
        self.version = "1.3.3"
        self.ref = (
            "https://figshare.com/collections/PanTHERIA_a_species-level_database_of_life_history_ecology_"
            "and_geography_of_extant_and_recently_extinct_mammals/3301274"
        )
        self.urls = {"data": "https://ndownloader.figshare.com/files/5604752"}
        self.citation = (
            "Kate E. Jones, Jon Bielby, Marcel Cardillo, Susanne A. Fritz, Justin O'Dell, C. David L. "
        ...
        self.description = (
            "PanTHERIA is a data set of multispecies trait data from diverse literature sources "
        ...
        if parse_version(VERSION) <= parse_version("2.0.0"):
            ...
            self.cleanup_func_table = Cleanup(
                correct_invalid_value, nulls=["NA", "-999.00"]
            )
        else:
            self.cleanup_func_table = Cleanup(
                correct_invalid_value, missing_values=["NA", "-999.00"]
            )
    ...
    def download(self, engine=None, debug=False):
        ...
        engine.auto_create_table(
            Table("species", cleanup=self.cleanup_func_table), filename=file_name
        )
